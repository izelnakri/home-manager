-- TODO: REFACTOR MOVE TO Option.peek from Option.peek_all
-- vim.fn.exists('&foo')
-- vim.fn.exists()

-- :opt returns all options
-- :get , :set is really for "options" HAS NOTHING TO DO WITH VARIABLES OR BUFFER/WINDOW PROPS

---@class Option
---@field allows_duplicates boolean
---@field commalist string
---@field default any
---@field flaglist boolean
---@field global_local boolean
---@field last_set_chan number
---@field last_set_line_nr number
---@field last_set_sid number
---@field name string
---@field scope "global" | "win" | "buf"
---@field shortname string
---@field type "number" | "boolean" | "dictionary" | "list" | "string"
...
Option.info = vim.api.nvim_get_all_options_info

--- @param reference? Buffer | Window | Option | number
function Option.peek_all(reference)
  local reference = {}
  if reference == nil or type(reference) == "string" then
  end

  -- Should be Option.peek_all(global), Option.peek_all(buffer), Option.peek_all(window)
  -- TODO: scope: "window" should get *all* options of *all* windows
  -- TODO: scope: "buffer" should get *all* options of *all* buffers

  -- if scope == nil then
  --   error("Option.peek_all($scope) requires a $scope when called!")
  -- end
  --
  -- -- TODO: IT DOESNT RETURN THE ACTUAL VALUE!!
  -- -- commalist, flaglist, global_local
  --
  local options = vim.api.nvim_get_all_options_info() -- NOTE: This is for current buffer
  local result = {}
  for key in Object.keys(options) do
    local metadata = options[key]

    if metadata.scope == "global" then
      result[key] = vim.g[key]
    elseif metadata.scope == "buf" then
      result[key] = vim.bo[key]
    elseif metadata.scope == "win" then
      result[key] = vim.wo[key]
    end
  end
